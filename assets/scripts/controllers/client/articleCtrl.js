angular.module('ototaihyundaiApp').controller('articleCtrl', function ($scope, $rootScope, $sce, $location, baseService) {
    ...
    function init() {
        ...
        baseService.POST(controller, param).then(function (response) {
            ...
            var obj = {
                ArtID: $scope.artOrPro.ArtID
            };
            $scope.relatedPost = $scope.artOrPro.RelatedPost;
            console.log($scope.relatedPost);
        }, function (err) {
...
//        var x = document.getElementById('img');
        $(this).css({
            'transform-origin': ((event.originalEvent.layerX - $('#img').offset().left) / $(this).width()) * 100 + '% ' + ((event.originalEvent.layerY - $('#img').offset().top) / $(this).height()) * 100 + '%'
        });
    }
